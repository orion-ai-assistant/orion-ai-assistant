...
def build_demo(
    ...
    def _gen_core(
        ...
        status_msg = f"Done. Server time {elapsed:.2f} seconds."
    ...
    def _gen_settings():
        with gr.Accordion("Generation Settings (optional)", open=False):
            ...
            se = gr.Number(
                ...
                label="Seed",
                precision=0,
                info="Set to 0 for random, >0 for deterministic generation.",
            )
        return ns, gs, dn, sp, du, pp, po, se
...
"""
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("Voice Clone"):
                ...
                def _clone_fn(
                    text, lang, ref_aud, ref_text, instruct, ns, gs, dn, sp, du, pp, po, se
                ):
                    return _gen(
                        text=text,
                        language=lang,
            ...
            with gr.TabItem("Voice Design"):
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        with gr.Accordion("Custom Instruct (optional)", open=False):
                            vd_instruct = gr.Textbox(
                                label="Custom Instruct",
                                lines=2,
                                placeholder="Add extra style instructions (e.g. 'Sad voice', 'High energy'). Combined with dropdown selections above.",
                            )
                ...
                def _build_instruct(groups, custom_instruct=None):
                    """Extract instruct text from UI dropdowns and custom text.
                ...
                def _design_fn(text, lang, ns, gs, dn, sp, du, pp, po, se, custom_instruct, *groups):
                    return _gen(
                        text=text,
                        language=lang,
                        ref_audio=None,
                        instruct=_build_instruct(groups, custom_instruct),
                        num_step=ns,
                        guidance_scale=gs,
                        denoise=dn,
                        speed=sp,
...
def main(argv=None) -> int:
    ...
    if args.tokenizer_device:
        logging.info("Moving audio tokenizer to %s ...", args.tokenizer_device)
        model.audio_tokenizer = model.audio_tokenizer.to(args.tokenizer_device)
    print("Model loaded.")
