...
def run_inference_batch(
    ...
    **gen_kwargs,
    ...
    # Pre-calculate or use cached voice clone prompts
    prompts = []
    if any(p is not None for p in ref_audio_paths):
        preprocess_prompt = gen_kwargs.get("preprocess_prompt", True)
        for p, t in zip(ref_audio_paths, ref_texts):
            if p is None:
                prompts.append(None)
                continue
            
            cache_key = (str(p), str(t), bool(preprocess_prompt))
            if cache_key not in worker_prompt_cache:
                worker_prompt_cache[cache_key] = worker_model.create_voice_clone_prompt(
                    ref_audio=p,
                    ref_text=t,
                    preprocess_prompt=bool(preprocess_prompt),
                )
            prompts.append(worker_prompt_cache[cache_key])
            
    audios = worker_model.generate(
        text=texts,
        language=langs,
        voice_clone_prompt=prompts if prompts else None,
