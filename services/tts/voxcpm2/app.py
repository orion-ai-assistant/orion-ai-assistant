...
_I18N_TRANSLATIONS = {
    "en": {
        ...
        "vae_cpu_label": "CPU Codec Offload (save VRAM)",
        "vae_cpu_info": "Run Audio VAE on CPU instead of GPU. Saves ~1-2GB VRAM but slightly slower synthesis.",
        "seed_label": "Seed (for deterministic voice)",
        "seed_info": "Set a specific number to get the same voice every time. Use -1 for random.",
    ...
    "zh-CN": {
        ...
        "vae_cpu_label": "CPU 编解码器卸载 (节省显存)",
        "vae_cpu_info": "在 CPU 而不是 GPU 上运行 Audio VAE。可节省约 1-2GB 显存，但合成速度略慢。",
        "seed_label": "随机种子 (Seed)",
        "seed_info": "设置特定数字以每次获得相同声音。-1 表示随机。",
...
class VoxCPMDemo:
    def __init__(self, model_id: str = "openbmb/VoxCPM2", vae_device: str | None = None) -> None:
        ...
        # Prompt cache: referans ses encode sonucu saklanır, her seferinde yeniden encode edilmez.
        self._prompt_cache: Optional[dict] = None
        self._prompt_cache_key: Optional[str] = None  # (ref_wav_path, prompt_wav_path, prompt_text, denoise)

        # Eager load the model on startup instead of lazy load
        self.get_or_load_voxcpm()

    def get_or_load_voxcpm(self) -> voxcpm.VoxCPM:
        if self.voxcpm_model is not None:
            return self.voxcpm_model
        logger.info(f"Loading model: {self._model_id} (VAE Device: {self._vae_device})")
        # load_denoiser=False ekleyerek internetten ek model indirmesini engelliyoruz
    ...
    def _get_or_build_prompt_cache(
        ...
        """Referans ses encode işlemini cache'e alır. Aynı dosya tekrar kullanılırsa encode atlanır."""
        if audio_path is None and prompt_text_clean is None:
            # Zero-shot modu: cache gerekmez
            self._prompt_cache = None
            self._prompt_cache_key = None
            return None

        # Cache anahtarı: dosya yolu + prompt metin + denoise ayarı
        cache_key = f"{audio_path}|{prompt_text_clean}|{denoise}"
        if cache_key == self._prompt_cache_key and self._prompt_cache is not None:
            logger.info("[Cache HIT] Referans ses encode adımı atlanıyor (cache kullanılıyor).")
            return self._prompt_cache

        logger.info("[Cache MISS] Referans ses encode ediliyor (bu işlem birkaç saniye sürer)...")
        prompt_cache = current_model.tts_model.build_prompt_cache(
            prompt_text=prompt_text_clean,
            prompt_wav_path=audio_path if prompt_text_clean else None,
            reference_wav_path=audio_path,
        )
        self._prompt_cache = prompt_cache
        self._prompt_cache_key = cache_key
        logger.info("[Cache] Encode tamamlandı, cache kaydedildi.")
        return prompt_cache
    ...
    def generate_tts_audio(
        ...
        logger.info(f"Generating audio for text: '{final_text[:80]}...' (Seed: {seed})")
        if seed != -1:
            torch.manual_seed(seed)
            if torch.cuda.is_available():
                torch.cuda.manual_seed_all(seed)

        # Referans sesi cache'ten al veya bir kez encode et
        ...
        with torch.inference_mode():
            wav, _, _ = current_model.tts_model.generate_with_prompt_cache(
        ...
        # VRAM temizliği: birden fazla üretimde bellek birikimini önler
        import gc
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

        # Ses verisini düzleştir (flatten) ve numpy array olarak döndür. 
        # Bazı durumlarda [1, T] şeklinde gelebiliyor, bu da WAV yazarken hata verebilir.
        audio_data = wav.detach().cpu().numpy().flatten()

        return (current_model.tts_model.sample_rate, audio_data)
...
def create_demo_interface(demo: VoxCPMDemo):
    ...
    def _on_toggle_instant(checked):
        if checked:
            # Düğme açıldığında: Metin kutusunu göster, içini temizle (ASR yapma)
            return (
                gr.update(visible=True, interactive=True, value=""),
                gr.update(visible=False),
            )
        # Düğme kapandığında: Metin kutusunu gizle, kontrol talimatını geri getir
        return (
...
def run_demo(
    ...
    demo = VoxCPMDemo(model_id=model_id, vae_device=vae_device)
...
if __name__ == "__main__":
    ...
    run_demo(model_id=args.model_id, server_port=args.port, vae_device=vae_device)
