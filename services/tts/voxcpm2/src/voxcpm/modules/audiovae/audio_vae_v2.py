...
class AudioVAE(nn.Module):
    ...
    def decode(self, z: torch.Tensor, sr_cond: torch.Tensor = None):
        ...
        if self.sr_bin_boundaries is not None:
            ...
            if sr_cond is None:
                sr_cond = torch.tensor([self.out_sample_rate], device=device, dtype=torch.int32)
            else:
                sr_cond = sr_cond.to(device)
    ...
    def encode(self, audio_data: torch.Tensor, sample_rate: int):
        ...
        device = next(self.parameters()).device
        audio_data = self.preprocess(audio_data.to(device), sample_rate)
