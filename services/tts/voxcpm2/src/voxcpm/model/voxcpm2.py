...
class VoxCPM2Model(nn.Module):
    ...
    def _encode_wav(
        ...
        feat = self.audio_vae.encode(audio, self._encode_sample_rate).cpu()
    ...
    def from_local(
        ...
        tokenizer = LlamaTokenizerFast.from_pretrained(path, local_files_only=local_files_only)
        ...
        # Bellek Optimizasyonu: VAE ağırlıklarını doğrudan yükle ve belleği boşalt
        model.audio_vae.load_state_dict(vae_state_dict, strict=False)
        del vae_state_dict
        gc.collect()

        # Ana model ağırlıklarını yükle
        ...
        if vae_device is not None:
            print(f"Moving AudioVAE to device: {vae_device}", file=sys.stderr)
            model.audio_vae = model.audio_vae.to(vae_device)
