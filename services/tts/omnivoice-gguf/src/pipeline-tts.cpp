...
void pipeline_tts_llm_batched_ctx_free(PipelineTTS * pt, MaskgitBatchedCtx * ctx) {
    ...
    ctx->lm_gf           = nullptr;
    ctx->lm_text_ids     = nullptr;
    ctx->lm_shifted      = nullptr;
    ctx->lm_mask         = nullptr;
...
ov_status pipeline_tts_synthesize(PipelineTTS *         pt,
    ...
    if (params->on_chunk) {
        ov_status rc = tts_synthesize_long_stream_internal(
            ...
            synth_ref_rms, params->dump_dir, &cc, params->on_chunk, params->on_chunk_user_data, params->speed);
        if (cc.triggered) {
    ...
    std::vector<float> audio =
        tts_synthesize_long_internal(pt, pc, tok, text, lang, instruct, params->T_override, params->chunk_duration_sec,
                                     ...
                                     synth_ref_tokens, synth_ref_T, synth_ref_rms, params->dump_dir, &cc, params->speed);
