...
static bool tts_parse_request(const std::string & body, tts_request & req, std::string & err) {
    ...
    yyjson_val * steps = yyjson_obj_get(root, "steps");
    if (steps && !yyjson_is_int(steps)) {
        err = "'steps' must be an integer";
        yyjson_doc_free(doc);
        return false;
    }
    req.steps = steps ? (int)yyjson_get_sint(steps) : -1;

    yyjson_val * guidance_scale = yyjson_obj_get(root, "guidance_scale");
    if (guidance_scale && !yyjson_is_num(guidance_scale)) {
        err = "'guidance_scale' must be a number";
        yyjson_doc_free(doc);
        return false;
    }
    req.guidance_scale = guidance_scale ? (float)yyjson_get_num(guidance_scale) : -1.0f;

    yyjson_val * speed = yyjson_obj_get(root, "speed");
    if (speed && !yyjson_is_num(speed)) {
        err = "'speed' must be a number";
        yyjson_doc_free(doc);
        return false;
    }
    req.speed = speed ? (float)yyjson_get_num(speed) : 1.0f;
